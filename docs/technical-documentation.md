...
BCODMONT incorporates terms from pre-existing terms within OBO ontologies in an effort to facilitate linkage with outside data repositories as well as to minimize the burden for creation of new richly defined terms. Cross reference the [OBO Foundry hompage](http://www.obofoundry.org/). //TODO: @Kai - is it possible to give a short description of what characteristics of these OBO ontologies make them good for direct incorporation as opposed to the other OBO ontologies that we are mapping to, but not importing?
...
In order to re-use ontology terms from existing OBO ontologies  we need to import them into our repository. Currently BCO-SM makes use of various OBO ontologies, see the [OBO Ontologies Used](#obo-ontologies-used) section. 
